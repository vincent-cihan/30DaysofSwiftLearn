...
private let myEndpointClosure = { (target: LYHApi) -> Endpoint in
...
let Provider = MoyaProvider<LYHApi>(endpointClosure: myEndpointClosure, requestClosure: requestClosure, plugins: [networkPlugin], trackInflights: false)
...
///   - target: 网络请求
///   - completion: 请求成功的回调
func NetWorkRequest(_ target: LYHApi, completion: @escaping successCallback ){
...
///   - completion: 成功的回调
///   - failed: 请求失败的回调
func NetWorkRequest(_ target: LYHApi, completion: @escaping successCallback , failed:failedCallback?) {
...
///   - failed: 失败
///   - error: 错误
func NetWorkRequest(_ target: LYHApi, completion: @escaping successCallback , failed:failedCallback?, errorResult:errorCallback?) {
    ...
    Provider.request(target) { (result) in
        ...
        case let .success(response):
            do {
                let jsonData = try JSONSerialization.jsonObject(with: response.data, options: []) as! Dictionary<String, Any>
                print(jsonData)
                ...
                print("flag不为1000 HUD显示后台返回message"+"\(String(describing: jsonData[RESULT_MESSAGE]))")
